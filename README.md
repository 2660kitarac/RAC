...
領収書に押す印影を設定できます。設定した印影は**単票印刷・一括印刷・会員マイページの領収書すべて**に反映されます。

### 設定場所
設定（`/settings`）→ **電子印鑑**タブ（クラブ管理者のみ表示）

### 2つの設定方法
| 方法 | 説明 |
|---|---|
| **印影画像**（推奨） | 実印をスキャン／撮影した画像をアップロード。背景透過PNGが最もきれい。長辺400pxに自動リサイズ |
| **印影文字** | 最大8文字を入力すると朱色の丸印を自動生成（画像がない場合に使用） |

「電子印鑑を領収書に押印する」をOFFにすると従来どおり空の印枠が印字され、手押しの実印を押せます。

### 表示優先順位
`印影画像` > `印影文字から自動生成` > `グレーの空枠`

### セキュリティ
- 画像は `data:image/(png|jpeg|webp);base64,` 形式の data URL のみ許可（外部URL埋め込みによる情報漏洩・XSSを防止）
- サイズ上限 約1MB / 印影文字は8文字以内
- 自クラブの設定のみ変更可（地区スタッフは全クラブ可）

### DB カラム（`clubs`）
| カラム | 型 | 説明 |
|---|---|---|
| `stamp_image_url` | text | 印影画像（data URL） |
| `stamp_text` | text | 印影文字（最大8文字） |
| `stamp_enabled` | boolean | 押印を有効にするか |

マイグレーション: `migrations/0009_club_stamp.sql`
API: `PATCH /api/settings`（`{ target: 'stamp', stampEnabled, stampImageUrl, stampText }`）
コンポーネント: `src/components/receipts/ReceiptStamp.tsx`

---

## 領収書一覧の表示範囲

領収書は**例会ごとの管理**を基本とし、一覧には過去分をすべて並べません。

| 表示範囲 | URL | 内容 |
|---|---|---|
| 直近60日（既定） | `/receipts` | 直近60日以内に発行した領収書のみ |
| 例会ごと | `/receipts?meeting_id=xxx` | その例会の領収書のみ |
| 全期間 | `/receipts?scope=all` | すべての領収書（明示指定時のみ） |

- 一覧上部の表示範囲フィルターで例会を選択して切り替えられます
- 一覧の「関連例会」列をクリックするとその例会に絞り込まれます
- 例会詳細（`/meetings/[id]`）の収支タブから「この例会の領収書」「領収書を一括印刷」に直接遷移できます

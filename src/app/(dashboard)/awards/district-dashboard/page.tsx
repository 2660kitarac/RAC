...
export default async function AwardsDistrictDashboardPage() {
  ...
  if (!districtId) {
    return (
      <div className="p-4 sm:p-6 max-w-7xl mx-auto">
        <h1 className="text-2xl font-bold text-gray-900">表彰ダッシュボード</h1>
  ...
  const clubList = await db
    ...
    .from(clubs)
    .where(and(eq(clubs.districtId, districtId), eq(clubs.type, 'RAC'), eq(clubs.isActive, true), isNull(clubs.deletedAt)))
    .orderBy(clubs.name);

  // award_score_items / award_scores は D1 未定義テーブル（将来実装）
